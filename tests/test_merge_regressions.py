...
from src.scheduler_rules import ACTIVE_RULE_IDS, scheduler_rule_id_from_action_index
...
class MergeRegressionTests(unittest.TestCase):
    ...
    def test_thesis_sched_rule_baselines_profile_uses_three_fixed_rules(self):
        cfg = SimConfig()
        apply_experiment_profile(cfg, "thesis_sched_rule_baselines")

        self.assertEqual(cfg.TRAIN_SCHEDULER_MODES, tuple(f"RULE_{rule_id}" for rule_id in ACTIVE_RULE_IDS))
        self.assertEqual(cfg.TRAIN_MAINT_MODES, ("NONE",))
        self.assertFalse(maintenance_decisions_enabled(cfg))
        self.assertTrue(cfg.DISABLE_HEALTH_SYSTEM)
        self.assertEqual(cfg.TRAIN_JOBS_TARGET, 200)
    ...
    def test_fixed_rule_mode_parsing_and_agent_are_supported(self):
        self.assertEqual(canonical_scheduler_mode("RULE_3"), "RULE")
        self.assertEqual(fixed_rule_id_from_mode("RULE_3"), 3)
        agent = FixedRuleSchedulerAgent(rule_id=3)
    ...
    def test_build_rule_twt_uave_rows_aggregates_by_rule(self):
        rows = build_rule_twt_uave_rows([
            {"scheduler_mode": "RULE_1", "tard": 10.0, "uave": 0.5},
            {"scheduler_mode": "RULE_1", "tard": 14.0, "uave": 0.7},
            {"scheduler_mode": "RULE_3", "tard": 7.0, "uave": 0.9},
        ])

        self.assertEqual(len(rows), 2)
        self.assertEqual(rows[0]["rule_tag"], "R1")
    ...
    def test_plot_rule_twt_uave_outputs_png_for_three_rules(self):
        rows = [
    ...
    def test_ppo_scheduler_returns_active_rule_ids_and_internal_action_index(self):
        cfg = SimConfig()
        cfg.SCHEDULER_MODE = "PPO"
        agent = PPOSchedulerAgent(state_dim=15, cfg=cfg, rng=random.Random(0), device=torch.device("cpu"))
        _, rule_id, action_index, _, _ = agent.act_with_info(np.zeros(15, dtype=np.float32), explore=False)
        self.assertIn(rule_id, ACTIVE_RULE_IDS)
        self.assertEqual(rule_id, scheduler_rule_id_from_action_index(action_index))
